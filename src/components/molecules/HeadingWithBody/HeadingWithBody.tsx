...
const HeadingWithBody = ({ heading, body, isVertical = false }: HeadingWithBodyProps) => (
  <div
    className={clsx('mb-2 mt-5 flex flex-col md:mt-3 lg:my-4', !isVertical ? 'md:flex-row' : '')}
  >
    <Heading className="w-full text-center md:w-3/5 md:text-left">{heading}</Heading>
    <p
      className={clsx(
        'w-full text-center text-sm text-neutral-500 dark:text-gray-500 md:w-3/5 md:text-left lg:text-lg',
        isVertical ? 'mt-5' : ''
      )}
    >
      {body}
